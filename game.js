...
function getFlagInfo(level, z, x, y){
  if (level.map_id === INTRO) {
    if (x < 40) {
      return {message: ["Nebras Eastergg!", "Flechas para Andar,", "Spacebar para Pular."],
              finished: false};
    } else {
      return {message:["Parabens,", "Voce Conseguiu!"], finished:true};
    }
  }
  ...
  if (level.map_id === CUBE_FRAME) {
    if (Math.abs(x) < 100 && Math.abs(y) < 100) {
      ...
      if (seed == 0) {
        return {message: ["Voce vai ter que olhar", "um pouco mais longe", "do que isso!"],
              finished: false};
      }
      if (seed == 1) {
        return {message: ["Eu sei que voce consegue", "ir longe..."],
              finished: false};
      }
      if (seed == 2) {
        return {message: ["Aqui nao!"],
              finished: false};
      }
    } else {
  ...
  if (level.map_id === FIGURE_EIGHT) {
    ...
    if (score >= 3) {
      return {message:["Voce conseguiu!", "( "+ score + " / 3)"], finished:true};
    }

    return {message:["Continue!", "(" + score + " / 3)"], finished:false};
  }
  return {message:["Terminou!"], finished:true}
};
